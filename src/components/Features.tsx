...
const FeatureCard = ({ icon: Icon, title, description, delay }: { 
  ...
  return (
    <div 
      ref={ref} 
      className={`glass-morphism rounded-xl p-6 transition-all duration-700 ease-out ${
        inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
      }`}
      style={{ transitionDelay: `${delay}ms` }}
    >
      <div className="w-12 h-12 rounded-full bg-fintech-500/20 flex items-center justify-center mb-5">
      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center mb-16">
          <div className="inline-flex items-center px-3 py-1 rounded-full border border-white/10 bg-white/5 mb-4">
            <span className="text-xs font-medium text-white/80">Advanced Features</span>
          </div>
          <h2 
            ref={ref}
